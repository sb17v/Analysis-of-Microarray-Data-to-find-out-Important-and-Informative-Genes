...
for i=1:m         % discretize all elements of x to disc_micro(i,j) as -2,-1,0,1 and 2
    for j=1:f
        disc_factor(i,j)=((mean_row(i)-x(i,j))/standev_row(i));
    end
end
...
for i=1:m    %defined discrete label
    for j=1:f
        if(disc_factor(i,j)<=-1)
            disc_micro(i,j)=1;
        elseif((disc_factor(i,j)>-1)&&(disc_factor(i,j)<0))
            disc_micro(i,j)=2;
        elseif((disc_factor(i,j)>=0)&&(disc_factor(i,j)<=1))
            disc_micro(i,j)=3;
        else
            disc_micro(i,j)=4;
